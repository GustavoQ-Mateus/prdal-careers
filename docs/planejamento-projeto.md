...
- **Grafo de conhecimento estilo Obsidian na interface**: visualização em grafo do processo do candidato, com nós de vagas, candidaturas, skills, categorias e notas de histórico, e arestas de relação. Evolui o "mapa de vagas em grafo" para um grafo do conhecimento pessoal, aproveitando a classificação da Fase 3 (ADR 0011) e o corpus do RAG (Fase 4). Feature de front, forte para portfólio e vídeo. Depende dos dados existirem primeiro; vira spec própria, provavelmente após a Fase 4.
- **Mensagens de entrevista e pré-seleção**: novas entidades de domínio ligadas à candidatura, para o usuário registrar o andamento (mensagens trocadas, etapa de pré-seleção). Entrada manual no início; captura automática por integração com e-mail ou chat é escopo maior e posterior. Cada uma vira spec própria de domínio.
